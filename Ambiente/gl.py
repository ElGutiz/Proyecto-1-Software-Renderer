...
class Renderer(object):
    ...
    def load(self, filename, translate, scale, rotate, normalizar):
        self.loadModelMatrix(translate, scale, rotate)
        opcion = normalizar
        model = Obj(filename, opcion)        
        vertex_buffer_object = []
        self.FigureList = []
        
        for face in model.faces:
            if len(face) == 3:
                self.FigureList.append('T')
    ...
    def triangle(self):
        for i in self.FigureList:
            if i == 'C':
                ...
                xmin, xmax, ymin, ymax = bbox(A, C, D)
                
                for x in range(xmin, xmax + 1):
                    for y in range(ymin, ymax + 1):
                        P = V2(x, y)
                        w, v, u = barycentric(A, C, D, P)
